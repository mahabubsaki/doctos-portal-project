...
const CheckoutFrom = ({ service }) => {
    ...
    useEffect(() => {
        const createPaymentIntent = async () => {
            try {
                const { data } = await axios.post('http://localhost:5000/create-payment-intent', service, {
                    headers: {
    ...
    const handleSubmit = async (event) => {
        ...
        if (error1) {
            ...
            setMyError('')
            try {
                const { data } = await axios.put(`http://localhost:5000/add-payment?id=${service.id}`, {
    ...
    return (
        <>
            <form onSubmit={handleSubmit}>
                ...
                />
                <button type="submit" className="btn bg-green-500 border-0 block mx-auto mt-2" disabled={!stripe || !clientSecret}>
                    Pay
                </button>
            </form>
            {loading &&
            ...
            {myError && <p className="text-center text-red-500">{myError}</p>}
        </>
    );
};
